...
st.write("Tell us more about your own perception on current hot topics in monetary policy")
st.write("**DISCLAIMER**: Your answer will be treated according to non-profiling and anonymity principles")
### survey questions
survey_question_1 = st.text_input("1) From a scale from 1 to 10, how suitable would you rate the approach undertaken by the EU and US central banks to inflation trends?")
survey_question_2 = st.text_input("2) Is your country's general inflation levels (core + volatile component) normalized, i.e., below the target threshold?")
survey_question_3 = st.text_input("3) From a scale from 1 to 10, how likely would you rate a potential base-interest rate cut by the FED in June?")
survey_question_4 = st.text_input('4) To what extent do you agree with the statement: "the 2008 GFC has played a pivotal role in lifting the veil on the relevance of regulatory intervention and supervision over the banking system, as the Covid-19 pandemic demonstrated"?')
survey_question_5 = st.text_input("5) Based on your knowledge of international monetary policy, how would you define the Bank of Japan's YCC (yield-curve-control) sustainability in a long-term perspective?")
st.write("   ") 
st.write("   ")
