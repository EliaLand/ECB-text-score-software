...
st.write("This is the work of Elia Landini, Jessie Cameron & Lina Abril (Pantheon-Sorbonne University).")
if st.button("GitHub"):
...
st.write("## Monetary policy reports dataframe")
st.markdown("""
<div style="text-align: justify">
This section describes the preliminary text analysis performed on the data from the Federal Reserve (FED). The analysis includes calculating several textual features that provide insights into the complexity and structure of the statements. The NLTK package is a comprehensive library for natural language processing and it is used for:            
...
""",
unsafe_allow_html=True)
FED_df = pd.read_csv("https://raw.githubusercontent.com/EliaLand/FED-text-score-software/main/Datasets/FED_df_data.csv")
st.dataframe(FED_df)
st.write("   ") 
st.write("   ")
...
st.write("""
<div style="text-align: justify">
The presented dataframe provides an overview of textual data after after the deployment of a number of cleaning techniques. The Monetary Policy Reports are pre-processed by excluding administrative details on the webpage to focus on the core of the text. In addition, several additional steps were undertaken to clean the data prior to estimating the tone of the minutes. This includes following steps to preprocess text data using NLTK. The process includes tokenisation, stopword removal, and lemmatization.
</div>
""",
unsafe_allow_html=True)
st.write("   ")
FED_cleaned_df = pd.read_csv("https://raw.githubusercontent.com/EliaLand/FED-text-score-software/main/Datasets/FED_cleaned_df.csv")
...
st.write("""
<div style="text-align: justify">
We consider the text data from all of the monetary policy statements, remove redundant and irrelevant works, and then count the frequency of each word. Using libraries like WordCloud, Matplotlib, and Seaborn, along with the Streamlit interface, we visualise the most frequently occurring words to gain insights into the data.
</div>
""",
unsafe_allow_html=True)
st.write("   ")
...
FED_df_final = pd.read_csv("https://raw.githubusercontent.com/EliaLand/FED-text-score-software/main/Datasets/FED_df_final.csv")
...
- $\\delta_{FC}$ is a dummy variable equal to 1 from 2007 to 2009, 0 otherwise, to capture the effects of the financial crisis.
- $\\delta_{cov}$ is a dummy variable equal to 1 from 2020 to 2023, 0 otherwise, to analyse the Covid-19 period.
""")
st.write("   ")
st.markdown("""
<table>
